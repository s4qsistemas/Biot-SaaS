...
async function main() {
    ...
    const planBasico = await prisma.plan.create({ data: { nombre: 'Básico', limite_usuarios: 5, precio_mensual: 15000, activo: true } });
    const planPro = await prisma.plan.create({ data: { nombre: 'Pro', limite_usuarios: 20, precio_mensual: 50000, activo: true } });
    const planEnterprise = await prisma.plan.create({ data: { nombre: 'Enterprise', limite_usuarios: 999, precio_mensual: 150000, activo: true } });
    const planTrial = await prisma.plan.create({ data: { nombre: 'Trial 14 Días', limite_usuarios: 5, precio_mensual: 0, activo: true } });
    ...
    // ==========================================
    const empresaRoot = await prisma.empresa.create({
    ...
    // ==========================================
    const emp4 = await prisma.empresa.create({
    ...
    // ==========================================
    const emp5 = await prisma.empresa.create({

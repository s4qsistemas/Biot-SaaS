...
export default function ConfiguracionHorarioOT({ ot, onUpdate }) {
                        ...
                        <div key={dia.dia} className={`grid grid-cols-1 md:grid-cols-12 gap-4 items-center p-3 rounded-lg border transition-colors ${dia.habilitado ? 'bg-dark-surface border-dark-border' : 'bg-dark-bg border-transparent opacity-50 grayscale'}`}>
                            ...
                            <div className="col-span-2 flex justify-center">
                                <label className="relative inline-flex items-center cursor-pointer">
                                    <input type="checkbox" className="sr-only peer" checked={dia.habilitado} onChange={(e) => handleRutinaChange(index, 'habilitado', e.target.checked)} />
                                    <div className="w-9 h-5 bg-slate-700 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-emerald-500"></div>
                                </label>
                            </div>
                            <div className="col-span-2"><input type="time" disabled={!dia.habilitado} value={dia.apertura} onChange={(e) => handleRutinaChange(index, 'apertura', e.target.value)} className="w-full bg-dark-bg border border-dark-border rounded p-1.5 text-sm text-center text-white focus:border-blue-500 outline-none disabled:cursor-not-allowed" /></div>
                            <div className="col-span-2"><input type="time" disabled={!dia.habilitado} value={dia.cierre} onChange={(e) => handleRutinaChange(index, 'cierre', e.target.value)} className="w-full bg-dark-bg border border-dark-border rounded p-1.5 text-sm text-center text-white focus:border-blue-500 outline-none disabled:cursor-not-allowed" /></div>
                            <div className="col-span-3 flex justify-center"><input type="number" min="0" step="0.5" disabled={!dia.habilitado} value={dia.colacion_horas} onChange={(e) => handleRutinaChange(index, 'colacion_horas', e.target.value)} className="w-20 bg-dark-bg border border-dark-border rounded p-1.5 text-sm text-center text-white focus:border-blue-500 outline-none disabled:cursor-not-allowed" /></div>
                        </div>
                    ))}
                </div>
            </div>
                    ...
                    <div className="space-y-3">
                        {configuracion.excepciones.map((exc, index) => (
                            <div key={index} className={`grid grid-cols-1 md:grid-cols-12 gap-3 items-center p-3 rounded-lg border ${exc.habilitado ? 'bg-dark-surface border-blue-500/30' : 'bg-red-500/5 border-red-500/20'}`}>
                                <div className="col-span-2"><input type="date" value={exc.fecha} onChange={(e) => handleExcepcionChange(index, 'fecha', e.target.value)} className="w-full bg-dark-bg border border-dark-border rounded p-1.5 text-xs text-white outline-none focus:border-amber-500" /></div>
                                <div className="col-span-3"><input type="text" placeholder="Motivo (ej. Feriado Legal)" value={exc.motivo} onChange={(e) => handleExcepcionChange(index, 'motivo', e.target.value)} className="w-full bg-dark-bg border border-dark-border rounded p-1.5 text-xs text-white outline-none focus:border-amber-500" /></div>
                                <div className="col-span-2 flex justify-center items-center gap-2">
                                    <span className="text-[10px] text-txt-secondary">¿Se trabaja?</span>
                                    <label className="relative inline-flex items-center cursor-pointer">
                                        <input type="checkbox" className="sr-only peer" checked={exc.habilitado} onChange={(e) => handleExcepcionChange(index, 'habilitado', e.target.checked)} />
                                ...
                                <div className="col-span-4 flex items-center gap-2">
                                    <input type="time" disabled={!exc.habilitado} value={exc.apertura} onChange={(e) => handleExcepcionChange(index, 'apertura', e.target.value)} className="w-full bg-dark-bg border border-dark-border rounded p-1.5 text-xs text-center text-white outline-none disabled:opacity-30" />
                                    <span className="text-txt-secondary">-</span>
                                    <input type="time" disabled={!exc.habilitado} value={exc.cierre} onChange={(e) => handleExcepcionChange(index, 'cierre', e.target.value)} className="w-full bg-dark-bg border border-dark-border rounded p-1.5 text-xs text-center text-white outline-none disabled:opacity-30" />
                                    <input type="number" step="0.5" disabled={!exc.habilitado} title="Colación" value={exc.colacion_horas} onChange={(e) => handleExcepcionChange(index, 'colacion_horas', e.target.value)} className="w-16 bg-dark-bg border border-dark-border rounded p-1.5 text-xs text-center text-white outline-none disabled:opacity-30" />
                                </div>
